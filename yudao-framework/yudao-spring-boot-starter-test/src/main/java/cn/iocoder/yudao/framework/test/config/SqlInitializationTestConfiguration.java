 ...
 *
 * 为什么不使用 org.springframework.boot.autoconfigure.sql.init.DataSourceInitializationConfiguration 呢？它的作用是在应用程序启动时，自动创建和初始化数据库表，并可以选择在启动时插入一些初始数据。
 * 因为我们在单元测试会使用 spring.main.lazy-initialization 为 true，开启延迟加载。此时，会导致延迟 DataSourceInitializationConfiguration 初始化
